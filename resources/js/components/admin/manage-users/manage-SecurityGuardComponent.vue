<template>
    <div class="pt-4">
        ...
        <v-row class="pt-2">
            <v-col>
                ...
                <v-card elevation="2" v-if="!loading">
                    ...
                    <v-data-table :headers="headers" :items="getSecurityGuard" :items-per-page="10" class="elevation-1">
        ...
        <v-dialog v-model="dialog" width="500">
            <v-card>
                <v-card-title class="">
                    {{this.type == "add" ? 'Add SecurityGuard' :  'Update SecurityGuard'}}
                </v-card-title>
                <v-divider></v-divider>
                <v-container>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...mapGetters(["getSecurityGuard", "filterSecurityGuard"]),
            // filteredItems() {
            //     if (this.search) {
            //         return this.SecurityGuardList.filter((item) => {
        ...
        methods: {
            ...
            deleteUser() {
                ...
                axios.delete('/api/admin/users/remove/' + this.delId)
                    .then((res) => {
                        if (res.status == 200) {
                            this.getSecurityGuard.splice(this.deleteIndex, 1);
                        ...
                        this.$store.dispatch('fetchAllSecurityGuard');
                    })
            },
                        ...
                        axios.post(`/api/admin/users/add/${this.user_type}`, this.form)
                            .then((response) => {
                                ...
                                if (response.status == 200) {
                                    ...
                                    this.SecurityGuardList.unshift(response.data);
                        ...
                        this.form.post('/api/admin/users/update/' + this.form.user_id)
                            .then((res) => {
                                ...
                                if (res.status == 200) {
                                    ...
                                    this.dialog = false;
                                    this.IsAddUpdating = false;
                                    this.$store.dispatch('fetchAllSecurityGuard').then((res) => {
                                        this.SecurityGuardList = this.getSecurityGuard;
                                    });
    ...
    //    this.fetchDeparmentList();
            this.$store.dispatch('fetchAllSecurityGuard').then(() => {
