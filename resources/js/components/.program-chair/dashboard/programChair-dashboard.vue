<template>
    <v-row class="mt-4">
        ...
        <v-col cols="12" lg="9" md="9" :class="windowSize == 'Small' ? 'pt-6' : 'pt-0'">
          ...
          <router-view v-if="$route.name == 'monitorTeacher_id'"></router-view>
          <monitorTeachers v-else></monitorTeachers>
...
<script>
  import {
    ...
    const monitorTeachers = () =>
            import ("../../.monitor-teachers/monitorTeachersComponent");
    export default {
        data() {
            return {
        ...
        methods: {
            ...
            getDeparmentsData() {
                axios.get('/api/department/data/' + this.get_CurrentUser.department_id)
                    .then((res) => {
                        if (res.status == 200) {
